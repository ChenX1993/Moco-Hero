...
public class PlayerMove : MonoBehaviour {
    ...
    void MoveAndRotate(float h, float v)
    {
		...
		//h<0表示获取玩家向左的输入，玩家以moveSpeed的速度向左运动
		//if (h < 0) transform.Translate(Vector3.left * moveSpeed * Time.deltaTime*0.5f);	
		//h>0表示获取玩家向右的输入，玩家以moveSpeed的速度向后右运动
		//else if (h > 0) transform.Translate(-Vector3.left * moveSpeed * Time.deltaTime*0.5f);
		//if (h != 0.0f) animator.SetBool("isMove", true);
    }
